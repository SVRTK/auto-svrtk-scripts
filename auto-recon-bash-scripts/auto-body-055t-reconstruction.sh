...
echo "-----------------------------------------------------------------------------"
echo
echo "SVRTK for fetal MRI (KCL): auto thoBODYrax DSVR reconstruction for 0.55T TSE T2w fetal MRI"
echo
echo "-----------------------------------------------------------------------------"
echo "-----------------------------------------------------------------------------"
echo
...
if [ $# -ne 2 ] ; then
    ...
    if [ $# -ne 6 ] ; then
        echo "Usage: bash /home/auto-proc-svrtk/auto-body-055t-reconstruction.sh"
...
current_monai_check_path=${model_path}/monai-checkpoints-unet-global-loc-05t-2-lab
...
# if [ $motion_correction_mode -eq 1 ]; then
    ...
    do
        ...
        ${mirtk_path}/mirtk extract-label ${all_masks[$i]} recon-masks-body/mask-${jj}.nii.gz 1 4
...
#        ${mirtk_path}/mirtk nan average_mask_cnn.nii.gz 0
    ...
    ${mirtk_path}/mirtk prepare-for-monai res-svr-files/ svr-files/ reo-svr-info.json reo-svr-info.csv ${res} ${number_of_stacks} DSVR-output-${recon_roi}.nii.gz > tmp.log
    ...
    ${mirtk_path}/mirtk register-landmarks ${current_template_path}/mask-${z1}.nii.gz out-svr-reo-masks/mask-${z1}.nii.gz init.dof dof-to-atl-${recon_roi}.dof ${n_roi} ${n_roi}  ${current_template_path}/mask-${z1}.nii.gz ${current_template_path}/mask-${z2}.nii.gz ${current_template_path}/mask-${z3}.nii.gz  ${current_template_path}/mask-${z4}.nii.gz  out-svr-reo-masks/mask-${z1}.nii.gz out-svr-reo-masks/mask-${z2}.nii.gz out-svr-reo-masks/mask-${z3}.nii.gz out-svr-reo-masks/mask-${z4}.nii.gz  > tmp.log

    ${mirtk_path}/mirtk info dof-to-atl-${recon_roi}.dof
    
    recon_resolution=1.2
    ${mirtk_path}/mirtk resample-image ${template_path}/body-stack-reo-template-2023/ref-space.nii.gz ref.nii.gz -size ${recon_resolution} ${recon_resolution} ${recon_resolution} -interp BSpline 
    ...
    ${mirtk_path}/mirtk convert-image reo-DSVR-output-${recon_roi}.nii.gz reo-DSVR-output-${recon_roi}.nii.gz -rescale 0 5000 -short 
